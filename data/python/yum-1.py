...
def user_main(args, exit_code=False):
    errcode = None
    if 'YUM_profiler' in os.environ:
        if os.environ['YUM_profiler'] == 'cprofiler':
            errcode = cprofiler(main, args)
        if os.environ['YUM_profiler'] == 'hotshot':
...
def hotshot(func, *args, **kwargs):
    ...
    fn = os.path.expanduser("~/yum.profiler")
    profiler = hotshot.profilerile(fn)
    rc = profiler.runcall(func, *args, **kwargs)
    profiler.close()
    show_stats(hotshot.stats.load(fn))
    return rc

def cprofiler(func, *args, **kwargs):
    import cprofilerile, pstats
    fn = os.path.expanduser("~/yum.profiler")
    profiler = cprofilerile.profilerile()

    rc = profiler.runcall(func, *args, **kwargs)
    profiler.dump_stats(fn)
    show_stats(pstats.Stats(fn))
    return rc
