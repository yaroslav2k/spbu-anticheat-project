...
class mSIL(Base):
    ...
    def _add_parameter(self, result: fdv.FunctionDefinitionCollector.Result, typehint_prob: float = 0.5, value_prob: float = 0.3) -> None:
    ...
    def _generate_name(self, min_len: int = 1, max_len: int = 10) -> str:
        ...
        name_tail = ''.join(self.randomizer.choice(string.ascii_letters + string.digits + '_') for i in range(name_len-1))
...
class TypehintGenerator:
    ...
    @classmethod
    def _generate_compound_typehint_and_value(cls, compound_type: str, randomizer) -> Tuple[cst.Annotation, cst.CSTNode]:
        elements = []
        match compound_type:
            case "List":
                ...
                typehint = cst.Annotation(cst.Subscript(cst.Name("List"), [cst.SubscriptElement(cst.Index(cst.Name(basic_type)))]))
